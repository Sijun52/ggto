...
2. 모든 차트의 `source.params.epsilonBb < 0.005` — **모델 안에서 정확한 균형** (1.6). 9-max 10bb BB앤티 차트가 이 PC(6코어) 에서 **10분 안에** 생성된다 (9절).
...
기호: `n_h` = 클래스 h 의 콤보 수, `w2(h,y)` = 서로 겹치지 않는 (h,y) 콤보 순서쌍 수 (P2 7.1 `pairCounts`, Σ = 1326·1225), `w3(h,y,c)` = 서로 겹치지 않는 순서 3쌍 수 (Σ_{ordered} = 1326·1225·1128 = 1,832,455,600), `p2(y|h) = w2(h,y)/(n_h·1225)`, `p3(y,c|h) = w3(h,y,c)/(n_h·1225·1128)`.
...
2. **폴더 f 의 폴드 확률 = 1 − (폴드가 아닌 액션 확률)** — 보수(complement) 규칙. 그 액션 확률은 f 의 결정 시점에 경로상 **먼저 나온 활성 핸드**를 조건으로 한다:
   ...
   - 앞선 활성 상대가 둘이면 (히어로 + 잼 + 콜 1 뒤에 앉은 폴더) 두 번째를 **버리고** `(h, y)` 로 조건한다. `w4` 가 없기 때문이며, **이것이 이 모델의 유일한 근사**다 (번칭 무시는 모델의 정의이지 근사가 아니다 — 번칭은 폴더의 핸드를 조건으로 넣는 것이고, 여기서는 활성 핸드만 조건으로 넣는다).
   - 상한에 걸린 강제 폴드는 확률 1.
3. **질량 결함 `massDefect`** = `max_{x,a,h} |Σ_z P(z | h, x, a) − 1|`. 규칙 2 의 근사가 없으면 정확히 0 이다 (보수 규칙 + 잼·콜러 결합분포의 주변화 `Σ_c w3(h,y,c) = 1128·w2(h,y)` 로 증명 — 테스트 `model.test.ts` 가 n=3 에서 `< 1e-12` 를 고정한다). n ≥ 4 에서는 값을 `source.params.massDefect` 에 적는다. EV 는 `Σ_z P(z|h,x,a)` 로 나눠 정규화한다.
4. **폴더의 카드 제거 = 활성 핸드 기준만.** 폴더의 폴드 레인지가 남은 덱을 바꾸는 효과(번칭)는 없다. HRC 기본 열거 모드와 같은 가정이다.

n=2 에서 이 모델은 `tools/chart-gen/src/pushFold.ts` 의 HU 모델과 **동일**하다 (BB 폴드 = 1 − `w`-가중 콜). 5.2 의 회귀 게이트가 그 사실을 실측으로 고정한다.
...
`u_i(σ)` (한 판 전체 기준, bb/핸드) 를 포지션별로 `source.params.gameValueBb` 에 적고 `sumGameValueBb = Σ_i u_i` 를 적는다. 1.4-3 의 근사가 없으면 정확히 0 이다. n ≥ 4 에서 `|Σ| > 1e-3` 이면 리뷰에서 UNCERTAIN 으로 올린다 (게이트는 아니다 — 근사의 크기를 아는 것이 목적).
...
format/version, 크기 = 818,805·8, 각 레코드 `|share 합 − 65535| ≤ 2`, `w3 ≤ 1728`, **Σ_{ordered} w3 = 1,832,455,600** (다중도: 세 클래스 서로 다름 6, 둘 같음 3, 셋 같음 1), i=j=k 레코드 지분 세 값 동일, sha256 일치. 메모리 전개: `share` 169³ Float32Array(19.6MB) — 로드 시 각 트리플의 세 지분을 **합으로 나눠 정확히 1** 로 만든 뒤 순서쌍 6개(또는 3/1) 위치에 복사, `w3` 도 169³ Float32Array.
...
반복 t:
  ...
  정지: epsilon < 1e-5  또는  t = 4000
결과: epsilon < 0.005 이면 성공, 아니면 throw ExploitabilityGateError(설정, epsilon, iterations)
```
...
- **결정성**: 단일 스레드, 합산 순서 고정, 정지 규칙 결정적 → 같은 입력(표 2개 + 설정) 은 바이트 동일 출력. `iterations` 는 실제 돌린 값을 적는다.
...
### 5.1 모델 (`model.test.ts`, n=3·4, 계산 가벼움)
- (a) 모든 터미널·핸드에서 `Σ_i u_i(z) = 0` (`< 1e-12`).
- (b) n=3 에서 `massDefect < 1e-12`, `|sumGameValueBb| < 1e-9` (근사 없음 — 1.4-3).
- (c) n=4 에서 `massDefect`·`sumGameValueBb` 를 **출력**하고 상한 `massDefect < 2e-3` (초과 시 리뷰 UNCERTAIN, 게이트 조정 금지).
- (d) 트리 표 (1.1) 일치.
...
### 5.4 단조성·상식 (`nmax.test.ts`, n ∈ {6, 9}, bba1, 스택 {5, 10, 15})
...
```jsonc
{
  ...
  "source": {
    ...
    "params": {
      ...
      "algorithm": "CFR+ (regret matching+, linear averaging, alternating updates); stop at epsilon < 1e-5 or 4000 iterations",
      "iterations": 850,
      "equityTable": "equity169.json@sha256:…", "equitySamples": null,
      "equityTable3": "equity169-3way.bin@sha256:…", "equity3Samples": 100000,
      "epsilonBb": 8.1e-6, "nashConvBb": 2.3e-5, "exploitabilityBb": 2.6e-6,
      "gainsBb": {"UTG": 1.2e-6, …}, "gameValueBb": {"UTG": -0.031, …, "BB": -0.512}, "sumGameValueBb": 3e-7, "massDefect": 4e-5,
      "truncationGainBb": 0.0031, "truncationByPlayerBb": {"BB": 0.0022, …}, "truncationSamples": 2000,
      "limitations": ["chip EV only: ICM not modeled — bubble and final-table decisions differ", "push/fold only: no limp, no min-raise", "at most 2 callers per jam", "no card bunching from folded ranges", "equal in-hand stacks"]
    }
  },
...
| 9-max bba1 10bb 한 차트 (단일 워커, 절단 측정 포함) | **≤ 10 min** | 4.1 반복당 ≤ 2s 조밀 × ~200 + 희소 |
| 6-max 한 차트 | ≤ 2 min | 3-way 터미널 20 |
| 2-max 한 차트 | ≤ 2 s | HU 5,000 반복이 0.35s |
| 전체 사다리 360 (6 워커) | ≤ 6 h 벽시계 | 사이즈별 합 ≈ 12 min/(스택·앤티) × 45 / 6 |
| `collectPool` 전체 사다리 | ≤ 5 s, +RSS ≤ 400MB | 8절 |
| `data/ggto.db` | ≤ 200MB | 노드 ≈ 16,470 × ≈ 5~8KB |

예산 초과 시 순서: (1) 희소 축약·타입 배열 최적화, (2) 스택 사다리에서 `11, 13, 14, 17` 제거 (11단), (3) 그래도 초과면 `--antes bba1` 만 DoD 로 하고 나머지는 옵션. **게이트(ε, S, 절단 상한)는 건드리지 않는다.**
...
4. **번칭 없음.** 앞선 폴더들의 레인지가 덱을 바꾸는 효과를 무시한다 (HRC 기본 모드와 같다). 늦은 포지션의 넓은 오픈 레인지 경계는 실제보다 약간 넓게 나올 수 있다.
...
1. `npm run ci` exit 0 (fresh clone 포함, Rust 없음). `tools/chart-gen` 테스트: 3.4·5.1~5.4 전부 절 번호로 존재. `packages/trainer/test/migrate.test.ts`. 출력 전문 첨부.
