...
**3.4-R 압축 모드 실측 (R2, UNCERTAIN 1 해소)** — 같은 스팟(`Ks7h2hQc`, simple, pot 20 / stack 80)을
**같은 200 iteration** 으로 비압축·압축 각각 솔브해 루트 노드를 비교했다 (반복수를 고정하지 않으면
차이가 "다른 CFR 궤적" 이 섞인다):

| 항목 | 비압축 | 압축 |
|---|---|---|
| `.bin` | 625,178B | 379,312B (0.61×) |
| `estimate.memoryBytes` | 1,458,544B | 951,276B (0.65×) |
| 루트 `evAvgBb` 합 | 20.0000 (= 팟) | 19.9999 (= 팟, ±1e-4) |

콤보별 EV 차: **max 0.0751bb**, mean 0.02274bb, 도달>0 인 (액션,콤보) 60쌍 중 **6쌍이 0.05bb 초과**.
즉 **압축 모드의 오차는 P6 채점 임계(0.05bb)를 넘을 수 있다** — D7 의 "기본 비압축" 이 옳고,
`compressed: true` 는 명시 옵션으로만 둔다 (해시에 들어가므로 다른 결과로 캐시된다).
테스트: `packages/solver/test/integration.test.ts :: P4 3.4 compressed 솔브가 저장·로드·node 까지 돌고 EV 합 = 팟이다`.
