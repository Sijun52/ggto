...
2. **정규 JSON** (키 정렬, 부동소수는 `Float32Array` 를 그대로 hex 로): `{ v: 2, board: formatCards(cb), oop: hex(oop'), ip: hex(ip'), pot, stack, sizings, rake, compressed, solver }`. **`targetExploitabilityPct`·`maxIterations` 는 제외** — 정확도는 4.3 의 "≥" 비교로 처리한다. **(P5, R1 MINOR 7)** `solver` 는 솔버 id(`SOLVER_ID`) 다: `.bin` 은 그 엔진의 직렬화 형식이고 전략 자체도 엔진마다 다르므로, 넣지 않으면 엔진 교체 후 같은 키로 **다른 엔진의 결과**를 답한다. `configHash(cfg, solverId)` 는 인자를 **필수**로 받는다 (호출부가 조용히 잊지 못하게).
...
**3.4-R 압축 모드 실측 (R2 — 개발 에이전트 실험 + 리뷰어 재실험, UNCERTAIN 1 해소)**

개발 에이전트의 설계: 같은 스팟(`Ks7h2hQc`, simple, pot 20 / stack 80)을 **같은 200 iteration** 으로 비압축·압축 각각 솔브해 루트를 비교 —
`.bin` 625,178B → 379,312B (0.61×), `estimate` 1,458,544B → 951,276B (0.65×), `evAvgBb` 합 19.9999 (= 팟 ±1e-4),
콤보별 EV 차 max 0.0751bb · mean 0.0227bb · 도달>0 (액션,콤보) 60쌍 중 6쌍 > 0.05bb.

**리뷰어 판정 — 실험 설계의 한계와 바른 결론** (`docs/reviews/P4-round2.md` 3절, 스크립트 `compressed.mjs`/`evloss.mjs`):

- 반복수 고정은 **결정적 비교**로는 성립한다 (같은 설정을 두 번 돌리면 1326 배열이 **비트 단위로 동일** — 실측 maxΔ 0.0000). 그러나 그 차이를 "압축 오차" 라고 부를 수 없다: 같은 200회의 **비압축** 솔브를 더 수렴한 비압축 기준(target 0.05%, 360 iter)과 비교하면 max 0.2254bb · 10/60 쌍 > 0.05bb 다 — 압축과의 차이(0.075bb)의 3배다. 200회 시점의 두 솔브는 **둘 다 미수렴**이고, 차이의 대부분은 궤적 발산이지 저장 정밀도가 아니다.
- 바른 통제는 **같은 목표 정확도로 수렴시킨 뒤 고정밀 기준과의 거리를 비교**하는 것이다. 결과:

| 스팟 | 비교 (기준 = 비압축 target 0.05%) | maxΔev | meanΔev | >0.05bb |
|---|---|---|---|---|
| 턴 `Ks7h2hQc` | 비압축 0.1% vs 기준 | 0.241bb | 0.028bb | 5/60 |
| 턴 | **압축 0.1%** vs 기준 | 0.211bb | 0.027bb | 7/60 |
| 턴 | **압축 0.05%** vs 기준 | 0.107bb | 0.010bb | 2/60 |
| 플랍 `Ks7h2h` (77.6MB) | 비압축 0.1% vs 기준 | 0.060bb | 0.018bb | 6/102 |
| 플랍 | **압축 0.1%** vs 기준 | 0.229bb | 0.042bb | 26/102 |
| 플랍 | **압축 0.05%** vs 기준 | **0.222bb** | 0.036bb | **23/102** |

- 턴(작은 트리)에서는 압축이 문제가 아니다 — 비압축끼리의 목표 정확도 차이가 더 크다. **플랍에서는 압축이 목표를 0.05% 까지 낮춰도 비압축 기준에서 0.22bb 떨어진 채 남는다** (비압축은 0.1% 에서 이미 0.06bb 안으로 들어온다). i16 누적 리그렛의 양자화 바닥으로 보인다 — 큰 트리일수록 값의 범위가 넓어 스케일당 해상도가 떨어진다.
- **결론 (D7 유지 + 강화)**: 기본 비압축이 옳고 `compressed: true` 는 명시 옵션으로만 (해시에 들어가므로 다른 결과로 캐시된다). **P6 트레이너는 `compressed: true` 행을 채점에 쓰지 않는다** (P5.md D28). P5 폼은 압축 토글을 만들지 않는다.
- **P6 에 넘기는 사실**: 콤보별 **EV loss** (`max_a ev − ev[a]`) 도 솔브 사이에서 흔들린다 — 턴 스팟에서 비압축 0.1% vs 0.05% 의 maxΔ(evloss) 0.257bb, Perfect(≤0.05bb) 판정이 뒤집히는 (액션,콤보) 2/60. 채점 임계 0.05bb 는 **한 솔브 안의** 비교에서만 의미가 있고, 채점용 솔브의 목표 정확도는 P6 스펙이 이 표를 근거로 정한다 (플랍은 0.1% 에서 0.06bb 안).
- 테스트: `packages/solver/test/integration.test.ts :: P4 3.4 compressed 솔브가 저장·로드·node 까지 돌고 EV 합 = 팟이다` (경로가 도는지 + 회귀 상한 `maxΔev < 1bb`. 위 표의 수치는 게이트가 아니라 기록이다).
...
4. 판정 결과를 이 문서 3.5 에 **적고**, `SolveResultMeta.evBasis: 'stack_delta_from_node'` 를 항상 만족하도록 변환을 Rust 쪽 `node` 응답에서 끝낸다 (Node 가 오프셋을 계산하지 않는다). `cargo test` 에 "임의 내부 노드에서 Σ_players ev_node = 그 노드의 팟 (±1e-3 × pot)" 을 성질 테스트로 둔다.
   - **(P5 정정, R1 MINOR 10)** `cargo test` 가 `evprobe` 예제를 **실행하지는 않는다** — 예제는 손으로 돌리는 조사 도구(`cargo run --release --example evprobe`) 이고, 테스트에 있는 것은 같은 성질(`Σ ev = 노드의 팟`)의 단언이다. 위 문장의 "`cargo test` 가 부른다" 는 빌드 대상에 포함된다는 뜻이었다.
...
CREATE TABLE solve (
  ...
  config_json     TEXT NOT NULL,              -- 정규 JSON (3.3-2) **전체** — 재현·목록용. 레인지 hex 둘을 포함한다
                                              -- (행만으로 게임을 재현하고 해시를 다시 만들 수 있어야 한다, R1 MAJOR 5)
...
- **원자성**: 솔버는 `<hash>.bin.part` 에 쓰고 완료 후 rename. index 행은 rename **뒤에** INSERT. 기동 시 `.part` 는 지우고, index 에 없는 `.bin` 은 지우고, `.bin` 없는 행은 지운다 (`SolveCache.repair()` — 결과 로그). **(P5)** 실패·취소한 잡의 `.part` 는 기동까지 기다리지 않고 큐가 그 자리에서 지운다 (R1 MINOR 3). **(P5)** `repair()` 는 옛 정규 JSON(`v:1`, 솔버 id 없음) 행도 버린다 — 같은 요청이 이제 다른 해시를 내므로 다시 조회될 수 없다 (R1 MINOR 7).
- **LRU 축출**: 새 결과를 저장하기 **전에** `Σ bytes + 예상 bytes > cap` 이면 `last_used_at` 오름차순으로 삭제. 실행 중·로드 중 해시는 제외. cap 기본 **20GB**, `GGTO_SOLVE_CACHE_BYTES` 로 변경. 삭제는 로그 한 줄.
  - **정확히 말하면** `evictFor(bytes, active)` 는 **저장 직전**, 즉 새 결과가 아직 index 에 없을 때 부른다 (R1 MINOR 6): 축출 계산에 들어가는 `Σ bytes` 는 **그 새 결과를 뺀** 값이고, 새 결과의 크기는 인자로 온다. 그래서 "저장 후 총량이 cap 을 넘지 않는다" 는 성질이 성립하며, 인자를 잊으면 cap 을 새 결과 크기만큼 넘긴다.
...
- **잡당 솔버 프로세스 1개** (`ggto-solver-cli --solve`), 최대 **2**. 완료 시 `.bin` 저장 후 exit 0. 취소 = `cancel` RPC → **500ms** 안에 끝나지 않으면 `kill()` (Windows 는 TerminateProcess), 그리고 정상 종료 경로는 stdin close → 2s 대기 → kill. 메모리는 프로세스와 함께 확실히 돌아온다 — 라이브러리 내부 해제에 기대지 않는다.
  - **(P5, R1 MINOR 5)** 협조적 취소만으로는 부족하다: 큰 플랍은 CFR 반복 한 번이 초 단위라 반복 경계까지 몇 초가 걸리고, 그동안 사용자에게는 "취소가 안 먹는다" 로 보인다. `CANCEL_KILL_MS = 500` 타이머가 그 상한이다. 남는 `.part` 는 큐가 지운다 (4.2).
- **조회 데몬 1개** (`ggto-solver-cli --serve`), 상주. `load {hash, path}` 로 `.bin` 을 메모리에 올리고 (`GGTO_SOLVER_LOADED_BYTES` LRU), `node`/`runouts` 를 답한다. 죽으면 다음 요청이 재기동하고 로드는 멱등 재로드. 부팅 시 미리 띄우지 **않는다** (바이너리 없는 PC).
  - **`load` 는 해시만으로 멱등이 아니다** (R1 MAJOR 3): 로드 당시의 `(파일 크기, mtime)` 을 기억하고 다르면 **다시 읽는다**. 재솔브(REPLACE)·삭제 후 재솔브는 같은 해시로 다른 `.bin` 을 만들기 때문이다. 캐시 쪽도 파일을 바꾸기 직전에 `Solver.invalidate(hash)` → `unload` 를 보낸다 (메모리를 더 일찍 돌려받는 최적화이고, **정답의 보증은 데몬의 (크기, mtime) 검사**다 — 훅은 잊힐 수 있고 데몬 크래시·CLI 경로를 못 덮는다).
    - **(R2 MINOR 4, 수용)** 이 훅은 **fire-and-forget** 이다: 라우트가 `void solver.invalidate(hash)` 로 부르고 rename 을 기다리지 않으므로 `unload` 와 파일 교체의 **순서는 보장되지 않는다**. 보장하려 하면 캐시 쓰기가 데몬 응답에 묶여 데몬이 죽었을 때 캐시까지 멈춘다. 순서에 기대지 않는 것이 설계다 — 정답은 위의 (크기, mtime) 스탬프가 보증한다 (P4 R2 에서 M5 뮤턴트가 이것을 확인했다).
  - **상한은 `.bin` 파일 크기의 합**이다 (RSS 폴링 없음 — 결정적이고 공짜다). 실측 배수 (R2, UNCERTAIN 2): 파일 0.54MB → RSS +2.3MB (×4.32), 5.42MB → +13.3MB (×2.45), **327MB → +686MB (×2.10)**. 즉 파일 합 1GB ≈ RSS 2.1GB 이므로 **기본값을 파일 합 1GB 로 낮춘다** (옛 기본 2GB 는 RSS 4GB 를 뜻했다).
...
- 동시 실행 **2**. `estimating` 도 실행 슬롯을 쓴다 (트리 빌드가 메모리를 먹는다). 잡 하나가 끝나면 **슬롯과 메모리가 허락하는 만큼 계속** 뽑는다 — 한 번에 하나만 시작하면 큰 잡이 끝난 뒤 슬롯이 논다 (R1 MAJOR 4).
- **메모리 게이트**: `estimate` 가 돌려준 `memoryBytes` (요청 모드) 에 대해 `Σ(실행 중 잡의 memoryBytes) + 새 잡 ≤ 8GB` (`GGTO_SOLVE_MEMORY_BYTES`) 가 아니면 큐에서 기다린다. **단일 잡이 8GB 를 넘으면 즉시 `failed: TooLarge`** (사용자에게 사이징을 줄이라고). 조회 데몬의 로드 상한(파일 합 1GB ≈ RSS 2GB, 5.1)은 별도이고 합산하지 않는다 (상한이 고정이므로).
...
```ts
{
  ...
  perm: [number, number, number, number]   // 이 응답에 적용된 원본→정규 순열. 항등이면 정규 보드 공간 (7절)
}
```
...
`{ line, board, perm, cards: [{ card: "Ah", evOop, evIp, equityOop, strategyRoot: number[] }] }` — 49(턴) 또는 48(리버) 장.
`board` 는 **그 chance 노드의 보드**다 (데몬이 준다 — 시작 보드를 쓰면 리버 chance 에서 딜된 턴 카드가 사라진다, R1 MAJOR 1). 각 카드에 대해 `play(chance)` 후 루트 액션의 레인지 가중 평균 전략과 두 플레이어 평균 EV (bb, 노드 기준). 카드는 정규 보드 기준으로 계산 후 **역순열** (블로커 슈트가 뒤집힌다 — 테스트 필수). 데몬은 이 49회 평가를 한 요청 안에서 끝낸다 (벤치 9절).
...
export interface Solver {
  ...
  invalidate(hash: string): Promise<void>;                    // 그 해시의 .bin 이 바뀐다/사라진다 → 들고 있으면 버려라 (5.1)
}
...
GET    /api/solve/:hash/node?line=&board=&oop=&ip=&potBb=&stackBb=[&sizings=&compressed=&rakePct=&rakeCapBb=]
                                     → NodeResponse (5.4) · 404 NoSolve · 400 NoSuchLine · 400 HashMismatch
GET    /api/solve/:hash/runouts?line=… (같은 쿼리) → 5.5
...
- **설정 쿼리와 `:hash` 의 대조 (R1 MAJOR 2 — P5 계약의 첫 절)**: `perm` 은 캐시 행에 저장하지 않으므로(4.1) 요청의 설정에서 다시 계산한다. 그래서 **그 설정이 이 해시의 게임인지 검증해야 한다**: `configHash(buildConfig(query)) !== :hash` 면 **400 `HashMismatch`**. 검증하지 않으면 다른 게임의 `perm` 을 이 게임의 1326 배열에 적용해 뒤섞인 배열을 "원본 슈트" 라고 답한다 (R1 실측: `board=Ad5d5c` 로 200).
  - `board`·`oop`·`ip`·`potBb`·`stackBb` 는 **함께** 와야 한다. 일부만 오면 400 (조용히 정규 보드로 떨어지면 사용자가 자기 슈트로 답을 받았다고 오해한다). `sizings`(프리셋 이름, 기본 `simple`)·`compressed`(`1`)·`rakePct`/`rakeCapBb` 도 해시 입력이므로 같이 받는다.
  - R2 실측 (`route2.mjs`, 실제 데몬): 다섯 파라미터의 부분집합 30개 전부 400 · 다섯 전부 200 · 해시 재계산 비용 +0.40ms/req (쿼리 없음 1.86ms → 있음 2.26ms) · `compressed=1` 은 다른 게임이라 400 `HashMismatch` · 같은 집합의 다른 레인지 텍스트는 200 · 보드 카드 콤보를 **추가**한 텍스트(`…,KsQs`)는 원 레인지의 슈트 대칭이 깨져 다른 해시(400, 캐시 미스 — 오답 아님) · `board=2hKs7h` 순서도 그대로 돌아온다 (`2hKs7hQc`) · 턴 솔브의 리버 라인 `X-X/9d` → `board` 5장.
  - **알려진 구멍 (P5 로 이월, P5.md 12절 R2 MINOR 1~3)**: `POST /api/solve` 가 커스텀 `sizings` 객체를 받아들인다 (3.2 "API 는 프리셋만" 위반) — 그런 솔브는 쿼리로 사이징을 표현할 수 없어 **정규 모드로만** 열린다. `compressed` 는 문자 `1` 만 참이다 (`true` 는 거짓). `sizings=…` 만 있는 쿼리는 "쿼리 없음" 으로 취급된다.
  - **쿼리가 아예 없으면** 정규 보드 공간 그대로 답하고, 응답의 `perm` 이 항등 `[0,1,2,3]` 으로 그 사실을 알린다. P5 는 목록(`SolveListItem.boardCanonical`)에서 열 때 이 경로를 쓰고, 사용자가 표기를 고르면 쿼리를 붙인다.
- **응답 `board` 는 라인이 지목한 스트리트의 실제 보드다** (R1 MAJOR 1): 사용자가 보낸 표기 + 라인에서 딜된 카드(역순열됨). `line=X-X/Qc` 면 `street: "turn"`, `board: "Ks7h2hQc"` 다. 시작 보드로 덮어쓰면 딜된 카드가 사라진다. `runouts` 도 같다 (chance 노드의 보드 — 리버 chance 면 4장).
